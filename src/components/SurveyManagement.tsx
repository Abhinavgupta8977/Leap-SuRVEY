...
export function SurveyManagement() {
  ...
  const [newSurvey, setNewSurvey] = useState({
    ...
    primaryModule: 'ai-readiness' as 'ai-readiness' | 'leadership' | 'employee-experience',
    modules: ['ai-readiness'] as ('ai-readiness' | 'leadership' | 'employee-experience')[]
  });
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label>Additional Modules (Optional)</Label>
                  <div className="flex flex-wrap gap-2 p-3 border rounded-lg bg-gray-50">
                    {(['ai-readiness', 'leadership', 'employee-experience'] as const).map((module) => (
                      <button
                        key={module}
                        type="button"
                        onClick={() => {
                          if (module === newSurvey.primaryModule) return; // Can't remove primary
                          
                          if (newSurvey.modules.includes(module)) {
                            setNewSurvey({
                              ...newSurvey,
                              modules: newSurvey.modules.filter(m => m !== module)
                            });
                          } else {
                            setNewSurvey({
                              ...newSurvey,
                              modules: [...newSurvey.modules, module]
                            });
                          }
                        }}
                        className={`px-3 py-1 rounded text-sm transition-colors ${
                          newSurvey.modules.includes(module)
                            ? module === newSurvey.primaryModule
                              ? 'bg-blue-600 text-white cursor-not-allowed'
                              : 'bg-green-600 text-white hover:bg-green-700'
                            : 'bg-white text-gray-600 border hover:bg-gray-100'
                        }`}
                        disabled={module === newSurvey.primaryModule}
                      >
                  ...
                  <p className="text-xs text-gray-500">
                    Primary module is always included. Add additional modules for comprehensive assessments.
                  </p>
                </div>
              </div>
                    ...
                    <div key={module} className="text-sm">
                      ...
                      <span className="text-gray-600 ml-2">
                        {module === 'ai-readiness' ? 'Average % of positive responses (options 4 or 5)' :
                         ...
                         'Percentage of higher responses on the 10-point scale (7-10)'}
                      </span>
                    </div>
                  ))}
                </div>
